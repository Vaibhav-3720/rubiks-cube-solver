...
private:
    ...
    struct Node
    {
        ...
        Node(const T &_cube, int _depth, int _estimate)
...
public:
    ...
    IDAstarSolver(const T &_rubiksCube, const string &fileName)
    {
        ...
        if (!cornerDB.fromFile(fileName))
        {
            throw std::runtime_error(
                "Failed to load corner pattern database from \"" +
                fileName +
                "\". Generate it first (see README) or check the path "
                "is correct relative to the working directory.");
        }
    }
